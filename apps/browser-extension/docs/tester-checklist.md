# Checklist Testeur - StreamVolume Guard Hub

Cette checklist sert à tester StreamVolume Guard Hub proprement et à envoyer un retour utile si quelque chose ne fonctionne pas.

Important : StreamVolume Guard Hub ne collecte pas de logs automatiquement. Le diagnostic JSON est généré localement et doit être exporté puis partagé volontairement par le testeur.
...
- [ ] Noter la version de StreamVolume Guard Hub testée.
...
```text
chemin vers StreamVolume Guard Hub\dist\firefox
```
...
```text
chemin vers StreamVolume Guard Hub\dist\safari-source
```
...
- [ ] Cliquer sur l'icône StreamVolume Guard Hub.
...
- [ ] Sur Chrome, Brave ou Edge, vérifier que `Protéger cet onglet` choisit une source active cohérente.
- [ ] Sur TikTok, vérifier que la source active passe en capture onglet quand Chromium le permet.
...
- [ ] Noter si StreamVolume Guard Hub aide réellement à éviter de toucher au volume pendant le live.
...
- clic droit sur l'icône StreamVolume Guard Hub, puis `Options` ;
- ou page extensions du navigateur, puis détails de StreamVolume Guard Hub, puis options de l'extension.
...
- [ ] Reproduire le bug.
- [ ] Copier uniquement les erreurs liées à StreamVolume Guard Hub ou à l'audio.
...
- [ ] Activer le mode développeur.
- [ ] Trouver StreamVolume Guard Hub.
