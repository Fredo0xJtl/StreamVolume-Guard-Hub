(function initOptions(root) {
  ...
  function safeStatus(status) {
    ...
    return {
      ...
      targetRmsDb: finiteNumber(source.targetRmsDb, -120),
      maxBoostDb: finiteNumber(source.maxBoostDb, 0),
      ...
      contextState: String(source.contextState || ""),
      audioTrackCount: finiteNumber(source.audioTrackCount, 0),
      captureTrackState: String(source.captureTrackState || ""),
      captureMuted: Boolean(source.captureMuted),
  ...
  async function buildDiagnosticReport() {
    ...
    return {
      ...
      streamerDiagnostics: {
        ...
        maxBoostDb: activeTab.maxBoostDb,
        contextState: activeTab.contextState,
        audioTrackCount: activeTab.audioTrackCount,
  ...
  async function savePlatformProfile(primaryDomain, profileId) {
    ...
    await Settings.saveSettings({ domainProfiles: domainProfiles, targetRmsMode: "profile" });
  ...
  async function render() {
    const settings = await Settings.getSettings();
    targetRmsMode = settings.targetRmsMode || "profile";
