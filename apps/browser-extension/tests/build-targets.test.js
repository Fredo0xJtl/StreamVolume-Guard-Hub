...
test("public docs explain browser support and platform limits", () => {
  ...
  const legacyReadme = fs.readFileSync(path.join(root, "README.legacy.md"), "utf8");
  ...
  assert.match(readme, /StreamVolume Guard Hub/);
  assert.match(readme, /BrowserExtension/);
  assert.match(readme, /BrowserGain/);
  assert.match(readme, /ObserveOnly/);
  assert.match(readme, /README\.legacy\.md/);
  assert.doesNotMatch(readme, /github\.com\/Fredo0xJtl\/StreamVolume-Guard\/releases/);
  assert.doesNotMatch(readme, /streamvolume-guard-chromium-0\.1\.4\.zip/);
  assert.match(legacyReadme, /Archive historique extension/);
  assert.match(readme, /edge:\/\/extensions/);
  assert.match(readme, /release-assets/);
